            ...
            group.items.forEach((item,itemIdx)=>{
                ...
                if(isReferenceValue){
                    ...
                    if (isEnergy) {
                        ...
                        let precioPuro = item.precioUnitario > 0 ? item.precioUnitario : (item.cantidad > 0 ? item.costo / item.cantidad : 0);
                        extraCardHtml=`<div class="detail-item detail-item-box energy"><label>Métricas de Energía</label><div class="detail-row"><span>Energía Total Facturada:</span><strong style="color:#111;">${formatNumber(baseAgg.kpis.energiaTotal)} kWh</strong></div><div class="detail-row"><span>Costo Energía Pura por kWh:</span><strong style="color:#111;">${formatMoney4(precioPuro)}</strong></div><div class="detail-row"><span>Costo Efectivo por kWh (Total Fra. / kWh):</span><strong style="color:var(--primary);">${formatMoney4(baseAgg.kpis.costoPorKWh)}</strong></div><div class="detail-row dashed dashed-energy"><span>Energía Acumulada YTD:</span><strong style="color:#111;">${formatNumber(ytdAgg.kpis.energiaTotal)} kWh</strong></div></div>`;
